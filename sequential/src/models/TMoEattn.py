...
from .MoEattn import MoEAttenBlock, MoEClipCA, Router
...
class TMoEClipCA(MoEClipCA):
    def __init__(
        ...
        **kwargs
        ...
        self.selected_modal_in = selected_modal_in
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.hidden_size != self.gen_emb_size:
            self.porj = nn.Linear(self.gen_emb_size, self.hidden_size)
...
class TMoEClipCA_CO(MoEClipCA):
    def __init__(
        ...
        **kwargs
        ...
        if self.hidden_size != self.gen_emb_size:
            self.proj = nn.Linear(self.hidden_size, self.gen_emb_size)

        self.attn_gate = Router(self.hidden_size, 2)
    ...
    def forward(self, log_seqs, ori_emb, text_emb):
        ...
        if self.modal_gate is not None:
            ...
            # batch X seg_len X 2 X emb_size, 생성 모달 정보 선택(text or img)
            gen_mm = torch.stack((gen_emb, prompt_res), dim=-2)
            gen_mm = torch.gather(
                gen_mm, dim=2, index=top_k_idx.expand(-1, -1, -1, gen_mm.size(-1))
            ).squeeze(-2)

            # batch X seg_len X 2 X emb_size, 아이템 모달 정보 선택(text or img)
            mm_info = torch.stack((ori_emb, text_emb), dim=-2)
            mm_info = torch.gather(
                mm_info, dim=2, index=top_k_idx.expand(-1, -1, -1, gen_mm.size(-1))
            ).squeeze(-2)
